...
//This upper commented class we were using before adding the api-gateway service, since api-gateway service is handling now everything we need to change or remove some validations from our class hence we have commented it and created new class where nothing is repeated
 ...
 *
 * IMPORTANT CHANGE: JWT validation is removed from here.
 * Gateway is now the single auth layer — it validates JWT,
 * extracts userId and role, and injects X-User-Id and X-User-Role headers.
 * user-service trusts those headers completely.
 *
 * We still keep:
 * - BCrypt for password hashing (register/login)
 * - AuthenticationManager for login flow
 * - STATELESS sessions
 *
 * We removed:
 * - JwtAuthFilter from filter chain
 * - Per-endpoint security rules (gateway handles this)
 * - anyRequest().authenticated() (gateway already enforced this)
 */
@Configuration
@EnableWebSecurity
...
public class SecurityConfig {
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
            .csrf(AbstractHttpConfigurer::disable)
            .authorizeHttpRequests(auth -> auth
                // Gateway has already validated JWT and injected headers
                // All requests reaching here are already authenticated
                // So permit everything — let controllers handle role checks
